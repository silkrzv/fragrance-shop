document.addEventListener('DOMContentLoaded', () => {
  ...
  fetch('/fragrance_shop/api/controllers/products.php')
    ...
    .then(data => {
      ...
      let total = 0;
      let html = '<table class="cart-table"><thead><tr><th>Produs</th><th>Preț</th><th>Cantitate</th><th>Subtotal</th><th>Acțiuni</th></tr></thead><tbody>';
      ...
      for (const [productId, qty] of Object.entries(cart)) {
        ...
        if (isNaN(price)) {
          console.error(`Preț invalid pentru produsul ${product.id}:`, product.price);
          continue;
        }
  ...
  // Buton checkout (poți lega cu pagina de plată)
  checkoutBtn.addEventListener('click', () => {
    alert('Funcționalitatea de finalizare comandă nu este implementată încă.');
  });

  // Ascultă evenimentul custom 'cartUpdated' pentru a reîncărca coșul când se modifică în altă pagină
